...
test('every portal is reachable only by its own role (spec 56)', async () => {
  const storage = makeLocalStorage();
  installWindow(storage);
  (await import('../js/store.js'))._clearMemoryStore();
  await import('../js/firebase.js');
  ...
  const cases = [
    // [credentials, page the user is standing on, roles that page allows, expected landing]
    [['2026-09-001', 'Student@123', 'student'], 'teacher.html', ['teacher'], 'student.html'],
    [['2026-09-001', 'Student@123', 'student'], 'admin.html',   ['admin'],   'student.html'],
    [['teacher@activeplus.edu', 'Teacher@123', 'teacher'], 'admin.html', ['admin'], 'teacher.html'],
    [['admin@activeplus.edu', 'Admin@123', 'admin'], 'teacher.html', ['teacher'], 'admin.html']
  ];

  for (const [creds, page, allowed, landing] of cases) {
    await auth.signIn(...creds);
    window.location.pathname = `/${page}`;
    redirects.length = 0;
    assert.equal(auth.requireRole(allowed), null, `${creds[2]} must be refused ${page}`);
    assert.match(redirects.at(-1)[1], new RegExp(`^${landing}(#s=.*)?$`),
      `${creds[2]} on ${page} is sent to ${landing}`);
  }

  // and each role does get through to its own portal
  const own = [
    [['2026-09-001', 'Student@123', 'student'], 'student.html', ['student']],
    [['teacher@activeplus.edu', 'Teacher@123', 'teacher'], 'teacher.html', ['teacher']],
    [['admin@activeplus.edu', 'Admin@123', 'admin'], 'admin.html', ['admin']]
  ];
  for (const [creds, page, allowed] of own) {
    await auth.signIn(...creds);
    window.location.pathname = `/${page}`;
    const session = auth.requireRole(allowed);
    assert.equal(session.role, creds[2], `${creds[2]} reaches ${page}`);
  }
});
